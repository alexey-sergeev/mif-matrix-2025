...
class evaluations {
    ...
    function __construct( $data )
    {
        ...
        if ( is_string( $data ) ) {
            ...
            $p = new parser();
            $data = $p->get_arr( $data, array( 'section' => 'evaluations', 'att_name' => false ) );
            // $data = $p->get_arr( $data, array( 'section' => 'evaluations', 'nomarker' => false, 'att_name' => true ) );
        ...
        foreach ( $data as $item ) {
            ...
            $this->evaluations_arr = $this->get_evaluations( $item );
    ...
    private function get_evaluations( $data )
    {
        $arr = array();

        if ( empty( $data['evaluations']['data'] ) ) return array();
        // p($data);

        $n = 0;

        foreach ( $data['evaluations']['data'] as $item ) {
